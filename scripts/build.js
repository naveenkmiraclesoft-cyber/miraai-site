...
function render(locale, route, rootPage = false) {
  ...
  const faq = locale.faq.items.map((item, index) => `<article class="faq-item${index === 0 ? " is-open" : ""}"><h3><button type="button" aria-expanded="${index === 0}" aria-controls="answer-${index + 1}">${escape(item[0])}${icon("chevron")}</button></h3><div class="faq-answer" id="answer-${index + 1}"${index ? " hidden" : ""}><p>${escape(item[1])}</p></div></article>`).join("");
...
<body>
	...
	<main id="main">
		<section class="hero" id="overview"><div class="hero-aurora" aria-hidden="true"></div><div class="shell hero-grid">
			<div class="hero-copy"><p class="eyebrow">${escape(locale.hero.eyebrow)}</p><h1>${escape(locale.hero.title)}</h1><p class="hero-text">${escape(locale.hero.text)}</p><div class="hero-actions"><a class="button" href="${common.briefingUrl}">${escape(locale.briefing)} ${icon("arrow")}</a><a class="button button-secondary" href="#platform">${escape(locale.hero.secondary)}</a></div><ul class="hero-points">${locale.hero.points.map(point => `<li>${icon("check")}${escape(point)}</li>`).join("")}</ul></div>
			<div class="hero-product reveal"><div class="product-bar"><span></span><span></span><span></span><strong>miraAI</strong></div><div class="product-canvas"><p>${escape(locale.hero.visualTitle)}</p><div class="flow">${locale.hero.visualSteps.map((step, index) => `<div><span>0${index + 1}</span><strong>${escape(step)}</strong></div>`).join("")}</div></div></div>
		</div></section>
		...
		<section class="section case-section" id="use-cases"><div class="shell"><header class="section-heading reveal"><p class="eyebrow">${escape(locale.useCases.eyebrow)}</p><h2>${escape(locale.useCases.title)}</h2><p>${escape(locale.useCases.text)}</p></header><div class="gallery reveal" data-gallery><div class="gallery-viewport"><div class="gallery-track">${slides}</div></div><div class="gallery-controls"><button class="gallery-arrow" type="button" data-gallery-prev aria-label="${escape(locale.useCases.previous)}">${icon("arrow")}</button><div class="gallery-dots">${dots}</div><p class="gallery-status" aria-live="polite" data-status-template="${escape(locale.useCases.status)}"></p><button class="gallery-arrow" type="button" data-gallery-next aria-label="${escape(locale.useCases.next)}">${icon("arrow")}</button></div></div></div></section>
